...
const firebaseConfig = {
  apiKey: "AIzaSyA4wMVw-xfsQKQS4SqYifj4G8FwsMvhE9E",
  authDomain: "gaddoors-ad542.firebaseapp.com",
  projectId: "gaddoors-ad542",
  storageBucket: "gaddoors-ad542.firebasestorage.app",
  messagingSenderId: "132047410854",
  appId: "1:132047410854:web:0d31b744711fab87528667"
};
...
// DATA FROM src/data/products.js
const products = [
...
async function clearCollection(colName) {
  ...
  console.log(`Cleared ${snapshot.size} docs.`);
}

async function uploadImage(imagePath) {
    const fullLocalPath = path.join(__dirname, '../public/images', imagePath);
    if (!fs.existsSync(fullLocalPath)) {
        console.warn(`⚠️ Image not found localy: ${fullLocalPath}`);
        return null; // Skip upload
    }

    const fileContent = fs.readFileSync(fullLocalPath);
    const metadata = { contentType: 'image/jpeg' };
    
    // Store in 'products/category/filename'
    // imagePath is like 'studioDoors/foo.jpg'
    const storagePath = `products/${imagePath}`; 
...
async function seed() {
  try {
    ...
    console.log(`Seeding ${products.length} products with rich metadata...`);

    for (const p of products) {
        // Upload image first
        let downloadUrl = null;
        try {
            downloadUrl = await uploadImage(p.image);
        } catch (e) {
            console.error(`Error uploading image for ${p.name}:`, e.message);
        }

        const productData = {
          ...p,
          image: downloadUrl || p.image, // Use URL if uploaded, else keep local path (fallback)
